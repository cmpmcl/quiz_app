...
def carica_domande(contenuto_str):
    ...
    # Regex modificata per catturare il NUMERO (Gruppo 1) e il TESTO (Gruppo 2)
    # Cerca "DOMANDA:", spazi, cifre (catturate), spazi, resto del testo
    ...
    for line in lines:
        ...
        # Se trova una riga "DOMANDA: N Testo..."
        if match_domanda:
            # Salva la precedente
            ...
            numero_orig = match_domanda.group(1) # Cattura il numero (es. "1", "150")
            ...
            if match_opzione:
                ...
                # Continuazione testo domanda su più righe
...
if st.session_state.stato_quiz == 'setup':
    ...
    st.markdown("Carica il file `.txt`. Il sistema leggerà automaticamente i numeri delle domande (es. `DOMANDA: 15 ...`).")
    
    uploaded_file = st.file_uploader("Trascina qui il file", type=["txt"])
    ...
    if uploaded_file:
        ...
        domande_parse = carica_domande(testo)
        
        if domande_parse:
            st.success(f"✅ Trovate {len(domande_parse)} domande.")
            ...
            st.error("Errore formato file.")

# --- FASE 2: IN CORSO ---
elif st.session_state.stato_quiz == 'in_corso':
    
    # Calcolo tempo lato Python (Verità assoluta)
    ...
    # Iniettiamo HTML+JS per far scorrere il timer visivamente senza ricaricare la pagina
    timer_html = f"""
    <div style="
    ...
    <script>
        ...
        updateTimer(); // Update subito
        setInterval(updateTimer, 1000); // Update ogni secondo
    </script>
    """
    # Usiamo components.html con altezza fissa per visualizzarlo in cima
    ...
    # Titolo con ID Originale tra parentesi
    # Es: (15) Qual è la capitale...
...
# --- FASE 3: RISULTATI ---
    ...
    for i, dom in enumerate(st.session_state.domande_selezionate):
        ...
        # Titolo expander con numero originale
